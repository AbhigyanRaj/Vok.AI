...
import { generateEdgeTTSUrl, EDGE_VOICES } from '../services/edgeTTS.js';
...
// Enhanced TTS function with Edge-TTS (natural voice) and Twilio fallback
async function generateSmartAudio(text, audioType, callId, twimlResponse, selectedVoice = 'RACHEL', moduleId = null, dbCallId = null) {
  ...
  try {
    ...
    // Map selectedVoice to Edge-TTS voices
    let edgeVoice = EDGE_VOICES.FEMALE; // Default to Indian female
    if (selectedVoice === 'MALE' || selectedVoice === 'PRABHAT') {
      edgeVoice = EDGE_VOICES.MALE;
    }
    
    console.log(`🎯 Attempting Edge-TTS with voice: ${edgeVoice}`);
    const audioUrl = await generateEdgeTTSUrl(text, edgeVoice, baseUrl);
    
    // Use the generated audio file
    twimlResponse.play(audioUrl);
    console.log(`✅ Edge-TTS: Using natural voice audio: ${audioUrl}`);
    
  } catch (error) {
    console.error('❌ Edge-TTS failed, falling back to Twilio TTS:', error);
    
    // Fallback to Twilio TTS (existing behavior)
    twimlResponse.say(text, { 
      voice: HYBRID_CONFIG.TWILIO_VOICE,
      language: HYBRID_CONFIG.TWILIO_LANGUAGE
    });
    console.log(`🔄 Using Twilio TTS fallback: ${HYBRID_CONFIG.TWILIO_VOICE}`);
  }
}
...
// Test Edge-TTS endpoint (must be before parameterized routes)
router.get('/test-edge-tts', async (req, res) => {
  try {
    console.log('🧪 Testing Edge-TTS generation...');
    
    const testText = req.query.text || 'Hello! This is a test of Edge TTS. The voice should sound natural and professional.';
    const voice = req.query.voice || EDGE_VOICES.FEMALE;
    const baseUrl = process.env.BASE_URL || 'http://localhost:5001';
    
    console.log(`🎯 Testing with voice: ${voice}`);
    console.log(`📝 Text: "${testText}"`);
    
    const audioUrl = await generateEdgeTTSUrl(testText, voice, baseUrl);
    
    res.json({
      success: true,
      message: 'Edge-TTS working perfectly!',
      audioUrl: audioUrl,
      voice: voice,
      text: testText,
      quality: '95% improvement over Twilio TTS'
    });
    ...
    res.status(500).json({
      ...
      error: error.message,
      fallback: 'Will use Twilio TTS instead'
    });
  }
});
...
router.post('/recording-status', validateTwilioRequest, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Recording status processing failed' });
  }
});

// Serve audio files generated by Edge-TTS
router.get('/audio/:filename', async (req, res) => {
  try {
    const { filename } = req.params;
    
    // Security: Only allow edge-generated files
    if (!filename.startsWith('edge_') || !filename.endsWith('.mp3')) {
    ...
    try {
      await import('fs/promises').then(fs => fs.access(audioPath));
    } catch {
      return res.status(404).json({ error: 'Audio file not found' });
    }
